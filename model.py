...
# parameter for the image size
H = 224
W = 49
log_location = 'data/driving_log.csv'
...
right_camera = data.copy()[['right', 'steering']]
right_camera['steering'] = right_camera['steering'] - 0.2200
right_camera.rename(columns={'right': 'image'}, inplace=True)
...
flipped_set = combined_set.copy()
combined_set['flip'] = False

flipped_set['flip'] = True
flipped_set['steering'] = flipped_set['steering'] * -1
...
final_data = pd.concat([combined_set, flipped_set, 
                        combined_set.copy(), flipped_set.copy()])


# Crop image and remove scenery
...
model = Sequential([
        Lambda(lambda x: (x/ 127.5 - 1.),input_shape=(W,H,3)),
...
model.fit_generator(train_gen, X_train.shape[0], 
                    nb_epoch=10, validation_data=valid_gen,
                    nb_val_samples=X_valid.shape[0])

model_json = model.to_json()
with open("model.json", "w") as json_file:
    json_file.write(model_json)
print("Written to model.json")
